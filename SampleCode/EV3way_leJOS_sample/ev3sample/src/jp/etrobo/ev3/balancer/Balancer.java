...
/**
 * 2輪型倒立振子ライントレースロボット「EV3way-ET」用バランス制御プログラム。
 *
 * EV3way-ETのバランス制御には、サーボ制御(状態 + 積分フィードバック)という現代制御を適用しています。
 * バランス制御器の制御パラメータについてはユーザーハンドコード側で定義する必要があります。定義はクラス
 * jp.etrobo.balancer.BalancerParam のソースコード中で行います。
